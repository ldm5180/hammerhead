    ...
    switch (event->type) {
        ...
        case CAL_EVENT_MESSAGE: {
            ...
            if (rval.code == RC_WMORE) {
                g_log(BIONET_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "CAL Message event from '%s' contained an incomplete ASN.1 message", event->peer_name);
                break;
            } else if (rval.code == RC_FAIL) {
                // received invalid junk
                g_log(BIONET_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "CAL Message event from '%s' contained an invalid ASN.1 message", event->peer_name);
                break;
            } else if (rval.code != RC_OK) {
                g_log(BIONET_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "unknown error (code=%d) decoding CAL Message event from '%s'", rval.code, event->peer_name);
                break;
            }

            if (rval.consumed != event->msg.size) {
                g_log(BIONET_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "CAL Message event from '%s' contained junk at end of message, dropping (consumed %d of %d)", event->peer_name, (int)rval.consumed, event->msg.size);
                // break;
            }
