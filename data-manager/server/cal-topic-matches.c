...
int libbdm_cal_topic_matches(const char *topic, const char *subscription) {
    ...
    if (subscription[0] == 'H') {
        ...
        sub_off = bdm_split_hab_name_r(&subscription[2], 
                NULL, sub_bdm_id, sub_hab_type, sub_hab_id);
        if (sub_off < 0) return -1;

        topic_off = bdm_split_hab_name_r(&topic[2], 
                NULL, topic_bdm_id, topic_hab_type, topic_hab_id);
        ...
        if(sub_off && topic_off) {
            ...
            &&  0 == bionet_param_to_timeval(topic_params, "tsmax", &tv_max) ) {
                ...
                if( 0 == bionet_param_to_timeval(sub_params, "estart", &tv_start) ) {
                        tsStart = &tv_start;
                }
                if( 0 == bionet_param_to_timeval(sub_params, "eend", &tv_stop) ) {
                        tsEnd = &tv_stop;
                }
    ...
    if (subscription[0] == 'N') {
        ...
        sub_off = bdm_split_node_name_r(&subscription[2], 
                NULL, sub_bdm_id, sub_hab_type, sub_hab_id, sub_node_id);
        if (sub_off < 0) return -1;

        topic_off = bdm_split_node_name_r(&topic[2], 
                NULL, topic_bdm_id, topic_hab_type, topic_hab_id, topic_node_id);
        if (topic_off < 0) return -1;
    ...
    g_log(BDM_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "unknown publish topic '%s'", topic);
    return -1;
}
