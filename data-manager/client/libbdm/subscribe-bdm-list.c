...
int bdm_subscribe_bdm_list_by_name(const char *bdm_id) {
    ...
    // see if this subscriptions reveals any new bdms
    {
        ...
        while(g_hash_table_iter_next(&iter, &key, &val)) {
            if (bionet_name_component_matches((const char *)key, bdm_id)) {
                bionet_bdm_t * bdm = (bionet_bdm_t*)val;

                if (bdm_cache_lookup_bdm(bdm_id) == NULL) {
