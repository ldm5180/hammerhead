...
# Convert the POSIX epoch into a matplotlib date.
epoch_as_mpldate = matplotlib.dates.date2num(datetime.utcfromtimestamp(0))
...
def ftimeval_to_mpldate(ftv):
    """
    Converts from a "timeval-as-float" (floating-point seconds since the
    UNIX epoch) to a matplotlib date (floating-point days since 01-01-0001)
    """
    if getattr(ftv, '__iter__', None) == None:
        return matplotlib.dates.date2num(datetime.utcfromtimestamp(ftv))
    else:
        return map(ftimeval_to_mpldate, ftv)
...
if __name__ == "__main__":
    ...
    # Test float timevals to matplotlib dates
    x = ftimeval_to_mpldate(0)
    if x != 719163.0:
        exit("Couldn't convert UNIX epoch to a matplotlib date.")
    x = ftimeval_to_mpldate(946684800)
    if x != 730120:
        print x
        exit("Coulnd't convert DTN epoch to a matplotlib date.")
