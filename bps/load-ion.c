...
int load_ion(void) {
    ...
    // sm_set_basekey is optional
    bdm_bp_funcs.sm_set_basekey = (sm_set_basekey_t)dlsym(libbp_handle, "sm_set_basekey");
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find sm_set_basekey() in libbp.so: %s, ignoring error\n", error);
    }

    bdm_bp_funcs.bp_attach = (bp_attach_t)dlsym(libbp_handle, "bp_attach");
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_attach() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_interrupt() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_open() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_close() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_get_sdr() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_send() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_receive() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }

    // bp_add_endpoint is optional
    bdm_bp_funcs.bp_add_endpoint = (bp_add_endpoint_t)dlsym(libbp_handle, "bp_add_endpoint");
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_add_endpoint() in libbp.so: %s, ignoring error\n", error);
    }

    bdm_bp_funcs.bp_release_delivery = (bp_release_delivery_t)dlsym(libbp_handle, "bp_release_delivery");
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find bp_release_delivery() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find Sdr_malloc() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find sdr_begin_xn() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find sdr_cancel_xn() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find sdr_end_xn() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find Sdr_write() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find zco_create() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find zco_append_extent() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find zco_start_receiving() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find zco_receive_source() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find zco_stop_receiving() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find zco_source_data_length() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find writeErrMemo() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
    ...
    error = dlerror();
    if (error != NULL)  {
        fprintf(stderr, "couldn't find writeErrmsgMemos() in %s: %s\n", LIBBP_FILE_NAME, error);
        return -1;
    }
